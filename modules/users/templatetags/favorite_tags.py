...
@register.simple_tag
def get_fav_notation(object, user):
    content_type = ContentType.objects.get(app_label=get_app_name(object), model=get_model_name(object).lower())
    favorites = Favorite.objects.filter(object_id=object.id, user=user, content_type=content_type)
    if favorites.exists():
...
@register.simple_tag
def get_nofav_notation(object, request):
    content_type = ContentType.objects.get(app_label=get_app_name(object), model=get_model_name(object).lower())
    if not request.session.exists(request.session.session_key):
        request.session.create()
    if getattr(settings, 'ALLOW_ANONYMOUS', 'TRUE') == "TRUE":

        favorites = Favorite.objects.filter(object_id=object.id,cookie=request.session.session_key, content_type=content_type)
        if favorites.exists():
            fav_value = getattr(settings, 'NEGATIVE_NOTATION', 'Unfavorite')
        else:
            fav_value = getattr(settings, 'POSITIVE_NOTATION', 'Favorite')
